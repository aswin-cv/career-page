$(document).ready(function () {
  ...
  $('#degree').change(function() {
    if ($('#degree').val() == 'Other') {
      ...
      $('#other_degree').hide();
      var dept = {
        BTech:['Information Technology','Aeronautical Engineering','Automobile Engineering','Biotechnology','Civil Engineering',
        'Computer Science and Engineering','Electrical and Electronics Engineering','Marine engineering','Mechanical Engineering',
        'Electronics & Communication','Other'],

        BE:['Computer Science Engineering','Mechanical Engineering','Electronics and Communication Engineering','Civil Engineering','Biochemical Engineering','Chemical Engineering','Aerospace Engineering','Other'],
  ...
  $('#resume').change(function(e) {
    $('#resname').text(e.target.files[0].name)
    $('#check').show();
  });

  // hide the check svg if inputs are reset
  $('#reset').mouseup(function(){
    $('#resname').text(" Upload Resume ")
    $('#check').hide();
    $('.tag').empty()
    $('.tag').removeClass()
...
function getIp(callback) {
  ...
  function process() {
    ...
    if (document.getElementById('mobile').value.match(/[a-zA-Z]/)) {
      console.log("Error");
      document.getElementById('mobile').value = "";
      document.getElementById('mobile').classList.add("is-invalid");
      document.getElementById('mobile').placeholder = "Enter a number as Input"
      document.getElementById("mobile_error").innerHTML("Invalid Number");
    }
    else {
      if (phoneInput.isValidNumber()) {
        ...
        document.getElementById('mobile').value = ""
        document.getElementById('mobile').classList.add("is-invalid");
        document.getElementById('mobile').placeholder = "Invalid Number"
        document.getElementById("mobile_error").innerHTML("Invalid Number");
      }
    }
  ...
  //     document.getElementById('mobile').value = ""
  //     if (isNaN(mobilenum)){
  //       document.getElementById('mobile').placeholder = "Enter only numbers"
  //     }
  //     else {
  //       document.getElementById('mobile').placeholder = "Invalid Number"
  //     }
...
document.getElementById("email").addEventListener("keydown", process1, false);

  function process1() {

    if (!document.getElementById('email').value.match(/^[-!#$%&'*+\/0-9=?A-Z^_a-z`{|}~](\.?[-!#$%&'*+\/0-9=?A-Z^_a-z`{|}~])*@[a-zA-Z0-9](-*\.?[a-zA-Z0-9])*\.[a-zA-Z](-?[a-zA-Z0-9])+$/)) {
      // console.log("Error");
      // document.getElementById('email').value = "";
      document.getElementById('email').classList.add("is-invalid");
      document.getElementById('email').placeholder = "Enter a valid Email"
      document.getElementById("email").addEventListener("mousemove", process1, false)
        function process1(){
